...
function displayREPLHelp() {
  ...
  console.log("");
  console.log(
    `  ${chalk.cyan("/scan")}               ${chalk.dim("Scan for new unfollowers")}`,
  );
  console.log(
...
async function startRepl(showWelcome = true) {
  ...
  rl.on("line", async (line) => {
    ...
    const greetingCmds = greetings.map((g) => "/" + g); // handle /hello etc too

    if (greetings.includes(lowerInput) || greetingCmds.includes(lowerInput)) {
      console.log("");
      console.log(chalk.hex("#ff6b6b").bold("  I am not your friend."));
      console.log(
        chalk.white("  I am a tool designed to track your betrayal."),
      );
      console.log(chalk.dim("  (Run /check if you dare)"));
      console.log("");
      rl.prompt();
      return;
    }
